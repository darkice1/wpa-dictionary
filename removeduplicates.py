...
if __name__ == "__main__":
    pathlist = ['.', "Big_dictionary"]
    for path in pathlist:
        print(f"path:{path}")
        txt_files = [f for f in sorted(os.listdir(path)) if f.endswith('.txt') and f != 'all.txt']
        print(txt_files)
        ...
        with open(f'{path}/all.txt', 'w', encoding='utf-8', errors='ignore') as file:
